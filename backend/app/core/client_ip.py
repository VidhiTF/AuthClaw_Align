...
class ClientIPConfig:
    ...
    def from_environment(cls) -> "ClientIPConfig":
        ...
        if mode not in _VALID_MODES:
            raise ValueError("AUTHCLAW_FORWARDED_HEADER_MODE must be off, compare, or enforce")
        try:
            max_hops = int(os.getenv("AUTHCLAW_FORWARDED_FOR_MAX_HOPS", "8"))
        except ValueError as exc:
            raise ValueError("AUTHCLAW_FORWARDED_FOR_MAX_HOPS must be an integer") from exc
        ...
        if mode != "off" and not networks:
            raise ValueError("trusted proxy CIDRs are required when forwarded headers are enabled")
...
def resolve_client_ip(peer: str, forwarded_for: str, config: ClientIPConfig) -> str:
    ...
    try:
        ...
        if not tokens or len(tokens) > config.max_hops or any(not token.strip() for token in tokens):
            return canonical_peer
...
class TrustedProxyMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope.get("type") == "http" and scope.get("client"):
            ...
            if proposed != peer:
                event_backbone.increment_metric("trusted_proxy_resolution_changed_total")
