...
def upgrade() -> None:
    ...
    op.execute("DROP TABLE IF EXISTS approval_linkage_053")
    op.execute(
        """
        CREATE TEMP TABLE approval_linkage_053 ON COMMIT DROP AS
    ...
    op.execute(
        ...
        FROM approval_linkage_053 canonical
    ...
    op.execute(
        ...
        SET action_id = left(approval.action_id, 200) || '#superseded:' || approval.id::text,
            resolution_reason = CASE
                WHEN COALESCE(approval.resolution_reason, '') LIKE '%[migration-053:%'
                    THEN approval.resolution_reason
                ELSE concat_ws(
                    ' ', NULLIF(approval.resolution_reason, ''),
                    '[migration-053: duplicate of ' || duplicate.keeper_id::text || ']'
                )
            END,
