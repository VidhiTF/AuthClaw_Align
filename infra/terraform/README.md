...
SQS audit transport requires `internal_tls.enabled = true`; Terraform rejects a
plaintext gateway-to-producer path. EC2 capacity uses IMDSv2 for the ECS agent and
sets `ECS_AWSVPC_BLOCK_IMDS=true` so `awsvpc` tasks cannot retrieve the container
instance profile.
