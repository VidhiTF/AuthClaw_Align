...
| Producers use canonical audit-record UUIDs | LOCAL-PASS | Backend and gateway SQS FIFO contract tests require canonical UUIDs. Legacy agent events are explicitly kept on Kafka until the agent emits ACL-21 canonical committed audit records. |
| Tenant ordering, deduplication and prior-hash behavior are equivalent | LOCAL-PASS | `audit_consumer/local_e2e.py --real-adapters` traverses backend producers, Redpanda/LocalStack consumers and ClickHouse; both transports produce identical durable IDs, tenant sequences and chain heads. |
...
| Retry, visibility renewal and DLQ behavior fail safely | LOCAL-PASS | `audit_consumer/tests/test_transport_sqs_fifo.py`; real-local LocalStack evidence observes repeated poison delivery without deletion and queue-policy redrive to the FIFO DLQ. |
...
| Task-role IAM permissions are separated and least privilege | LOCAL-PASS | Speculative SQS plan includes backend/gateway producer-only send/metadata roles and consumer-only receive/delete/visibility/metadata role; agent receives no canonical SQS producer role. Live IAM simulation remains deployment-only. |
