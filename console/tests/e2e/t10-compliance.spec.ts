...
const control = {
  ...
  score: 84.9, status: "partial", evidence: ["10000 audit events"], gaps: ["Assessment expired; review required"],
  evidence_assessment: {
...
const scoreResponse = {
  overall_score: 84.9, readiness_level: "monitor", calculation_version: version, generated_at: generatedAt,
  frameworks: ["SOC2", "GDPR", "HIPAA"].map((framework) => ({
    framework, score: 84.9, readiness_level: "monitor", calculation_version: version,
  ...
  trust_summary: {
    ...
    counts: { verified: 0, in_progress: 1, planned: 0 },
    verified: [], in_progress: [{ ...control, framework: "SOC2" }], planned: [],
  },
};
...
test("compliance exposes blocked evidence, legacy history, and method boundaries", async ({ page }) => {
  ...
  await page.route("**/api/compliance-scores/history?*", (route) => route.fulfill({ json: { items: [
    { framework: "SOC2", snapshot_date: "2026-09-18", overall_score: 84.9, calculation_version: version },
  ...
  await expect(page.getByText("Activity Diagnostics", { exact: true })).toHaveCount(2);
});
...
test("failed score refresh removes prior assessment and Trust Summary", async ({ page }) => {
  ...
  await expect(page.getByText("84.9%", { exact: true })).toHaveCount(0);
});
...
test("overview shows server readiness and clears it when the next request fails", async ({ page }) => {
  ...
  await expect(page.getByRole("cell", { name: "MONITOR", exact: true })).toHaveCount(3);
