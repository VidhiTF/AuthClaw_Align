        ...
        {loading ? (
          ...
          <div className="divide-y divide-[#E6E9F0]">
            {visibleRequests.map((request) => {
              const latestInvitation = request.history.find((entry) => typeof entry.metadata?.delivery === "string");
              const canRetryInvitation = request.status === "APPROVED" && latestInvitation?.metadata?.delivery === "failed";
              return (
              <article key={request.reference} className="px-5 py-5">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                  ...
                  {canRetryInvitation && (
                    <button
                      type="button"
                      onClick={() => void transition(request, "INVITED")}
                      disabled={busyReference === request.reference}
                      className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800 transition hover:bg-amber-100 disabled:opacity-50"
                    >
                      {busyReference === request.reference ? <Loader2 className="h-4 w-4 animate-spin" /> : <MailWarning className="h-4 w-4" />}
                      Retry invitation
                    </button>
                  )}
                </div>
