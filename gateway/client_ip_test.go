...
func TestClientIPMalformedAndOverlongChainsFallBack(t *testing.T) {
	for _, header := range []string{"garbage", "198.51.100.9:443", "[2001:db8::1]:443", "198.51.100.9,", "192.0.2.1,198.51.100.2,203.0.113.3"} {
		...
		if got != "10.0.0.4" {
			t.Fatalf("header %q got %q", header, got)
		}
	}
}
...
func TestClientIPSupportsCanonicalIPv6AndCompareMode(t *testing.T) {
	...
	if got != "10.0.0.4" {
		t.Fatalf("compare mode enforced proposed address: %q", got)
	}
}
...
func TestClientIPConfigurationRequiresCanonicalCIDRs(t *testing.T) {
	...
	if _, err := newClientIPResolverFromEnv(); err == nil {
		t.Fatal("expected invalid non-canonical CIDR")
	}
}

func TestClientIPCompareMeasuresLegacyMismatchWithoutTrustingIt(t *testing.T) {
	before := trustedProxyLegacyMismatchTotal.Load()
	got := resolver(t, "compare", 8, "10.0.0.0/24").resolve("10.0.0.4:443", "192.0.2.66, 198.51.100.9")
	if got != "10.0.0.4" || trustedProxyLegacyMismatchTotal.Load() != before+1 {
		t.Fatal("comparison must count legacy mismatch and retain socket identity")
	}
}
