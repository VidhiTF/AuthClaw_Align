"""Centralized structured logging and payload redaction for the agent service."""
...
_request_id = contextvars.ContextVar("agent_log_request_id", default="")
_trace_id = contextvars.ContextVar("agent_log_trace_id", default="")
_operation = contextvars.ContextVar("agent_log_operation", default="")
...
class SafeJSONFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        payload = {
            ...
            "service": os.getenv("AUTHCLAW_SERVICE", "agent").strip() or "agent",
...
class SafeJSONStream:
    """Convert legacy print output into redacted structured log events."""

    def __init__(self, destination, level: int = logging.INFO):
    ...
    def write(self, value: str) -> int:
        ...
        if message:
            record = logging.LogRecord("agent.legacy_stdout", self.level, "", 0, message, (), None)
            self.destination.write(SafeJSONFormatter().format(record) + "\n")
        return len(value)
...
def configure_safe_logging() -> None:
    ...
    # Pytest owns its capture streams. Production captures legacy print paths
    # so older nodes cannot bypass structured redaction.
    if "pytest" not in sys.modules:
        if not isinstance(sys.stdout, SafeJSONStream):
            sys.stdout = SafeJSONStream(original_stdout, logging.INFO)
        if not isinstance(sys.stderr, SafeJSONStream):
            sys.stderr = SafeJSONStream(original_stderr, logging.ERROR)
