...
3. Provision every ARN from required_secret_arns using the organization's secure external process. Never put values in tfvars, Terraform outputs, build args or repository files. Database URLs must select the intended DB role. The gateway URL must explicitly use `sslmode=verify-full` with a trusted CA and a hostname matching the database certificate; other clients must use their strongest supported equivalent.
