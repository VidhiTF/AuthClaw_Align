...
| SOC 2 CC7.1 | Detect vulnerabilities, secrets, and insecure infrastructure before release. | Kunal | Kunal | Built-in | `.github/workflows/ci.yml`; `infra/security/COMPLIANCE_HARDENING_RUNBOOK.md#vulnerability-management`; [ACL-7 audit evidence](https://github.com/AgentsArchitects/AuthClaw/pull/7) | Relevant changes after each master push; every release | Retain real CodeQL, dependency/filesystem, secret, IaC, and release-image scan outputs; local or example manifests are not release evidence. |
...
| GDPR Article 15 | Support authenticated, tenant-isolated access requests and signed subject-data exports. | Kunal / Vidhi | Authorized governance reviewer | Built-in | `backend/app/services/data_subject_requests.py`; `backend/tests/test_endpoints.py::test_data_subject_request_lifecycle_authorization_and_isolation`; signed export and audit records | Per request; evidence review per release | Organizational identity-verification and secure artifact-delivery procedures remain required. |
| GDPR Article 17 | Support erasure and retention workflows for personal data and derived records. | Kunal / Vidhi | Authorized governance reviewer | Built-in | `backend/app/services/data_subject_requests.py`; `backend/tests/test_endpoints.py::test_data_subject_request_lifecycle_authorization_and_isolation`; deletion and immutable audit records | Per request; monthly aging review | Review documented retained categories and legal exceptions for each completed request. |
